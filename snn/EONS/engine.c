...
static size_t *cls_idx[SNN_NUM_HW_CLASSES];
static size_t  cls_cnt[SNN_NUM_HW_CLASSES];
static const ascad_dataset_t *cached_ds = NULL;
...
static void build_class_index(const ascad_dataset_t *ds) {
    if (cached_ds == ds) return;
    ...
    for (size_t i = 0; i < ds->num_traces; i++) {
        uint8_t lbl = ds->labels[i];
        if (lbl < SNN_NUM_HW_CLASSES) cls_cnt[lbl]++;
    }
    for (int c = 0; c < SNN_NUM_HW_CLASSES; c++)
        cls_idx[c] = (cls_cnt[c] > 0) ? malloc(cls_cnt[c] * sizeof(size_t)) : NULL;

    size_t fill[SNN_NUM_HW_CLASSES] = {0};
    for (size_t i = 0; i < ds->num_traces; i++) {
        uint8_t lbl = ds->labels[i];
        if (lbl < SNN_NUM_HW_CLASSES && cls_idx[lbl])
            cls_idx[lbl][fill[lbl]++] = i;
    }
    cached_ds = ds;
}
...
void engine_evaluate_generation(candidate_t *pop, size_t population_size,
    ...
    for (int c = 0; c < SNN_NUM_HW_CLASSES; c++)
        if (cls_cnt[c] > 0) present_classes++;
    if (present_classes == 0) return;

    size_t batch = (size_t)present_classes * PER_CLASS_SAMPLES;
    size_t *batch_idx = malloc(batch * sizeof(size_t));
    if (!batch_idx) return;
    size_t  bpos = 0;
    for (int c = 0; c < SNN_NUM_HW_CLASSES; c++) {
        if (cls_cnt[c] == 0) continue;
        for (int k = 0; k < PER_CLASS_SAMPLES; k++) {
            size_t r = (size_t)rand() % cls_cnt[c];
            batch_idx[bpos++] = cls_idx[c][r];
        }
    }
    ...
    for (size_t i = 0; i < population_size; i++) {
        ...
        if (!net) { pop[i].fitness_score = -1e30f; continue; }

        float log_likelihood_sum = 0.0f;
        float out_probs[SNN_NUM_OUTPUTS];
        for (size_t b = 0; b < batch; b++) {
            size_t t = batch_idx[b];
            const float *trace = &ds->traces[t * ds->trace_length];
            snn_predict_proba(net, trace, ds->trace_length, out_probs);
