# Send-On-Delta Modulation Implementation

def delta_modulation(trace, threshold):
    reference = int(trace[0])
    for sample in trace:
        diff = int(sample) - reference
        
        if diff >= threshold:
            yield 1
            reference += threshold
        elif diff <= -threshold:
            yield -1
            reference -= threshold
        else:
            yield 0

def rebuild_trace(modulated_trace, init_val, threshold):
    current_value = int(init_val)

    for pulse in modulated_trace:
        if pulse == 1:
            current_value += threshold
        elif pulse == -1:
            current_value -= threshold
        yield current_value

#Hamming Weight Calculation for AES S-Box Outputs
import numpy as np
 # Precompute AES S-Box and Hamming Weight Lookup Table
AES_SBOX = np.array([
    
], dtype=np.uint8)

HW_LUT = np.array([bin(x).count("1") for x in range(256)], dtype=np.uint8)

def calculate_hw_labels(plaintext_byte_array, key_byte_array):
    sbox_out = AES_SBOX[plaintext_byte_array ^ key_byte_array]
    return HW_LUT[sbox_out]